...
class TestPermissionAPI(BaseAPITest):
    ...
    async def test_assign_role_to_permission_success(self, async_client, auth_patch):
        ...
        response = await self._assign_role_to_permission(
            async_client, auth_patch, permission["id"], role["id"]
        )
        assert response.status_code in (200, 204)

    @pytest.mark.negative
    async def test_assign_role_to_invalid_permission(self, async_client, auth_patch):
        role = await self._create_role(async_client, auth_patch)
        response = await self._assign_role_to_permission(
            async_client, auth_patch, 99999, role["id"]
        )
    ...
    async def test_assign_invalid_role(self, async_client, auth_patch):
        permission = await self._create_permission(async_client, auth_patch)
        response = await self._assign_role_to_permission(
            async_client, auth_patch, permission["id"], role_id=99999
        )
        assert response.status_code == 404

    @pytest.mark.negative
    async def test_forbidden_assign_role_to_permission(self, async_client, auth_patch):
        permission = await self._create_permission(async_client, auth_patch)
        role = await self._create_role(async_client, auth_patch)
        response = await self._assign_role_to_permission(
            async_client, auth_patch, permission["id"], role["id"], perms=["role:nonadmin"]
        )
        assert response.status_code == 403
